...
SailGauge.prototype = {
  ...
  updateSpeedOverGround: function(speed) {
    if (speed != null) {
      d3.select('#speed').text((ms2kn*speed).toFixed(1));
    }
  },
  ...
  updateApparentWindSpeed: function (speed) {
    d3.select("#apparentwindmarkertext")
      .text((ms2kn*Number(speed)).toFixed(0));
  },
  ...
  drawCompassRose: function (chart, size) {
    ...
    rose.append('circle').attr('cx', size / 2).attr('cy', size / 2).attr('r', size / 2 - 20).attr('fill', 'gray').attr('stroke-width', '2px');
    rose.append('path')
  ...
  drawBoat: function (chart) {
    var boat = chart.append('g').attr('id', 'boat').attr('transform', 'rotate(90 100 100) translate(140 -462) scale(2.6)')
    boat.append('path')
