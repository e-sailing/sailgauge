...
function getTrueWindAngle(speed, windSpeed, windAngle) {
  ...
  return Math.atan2(apparentY, -speed + apparentX) * (180 / Math.PI);
};
...
module.exports.createTrueWindAngleStream = 
  function(sogStream, apparentWindSpeedStream, apparentWindAngleStream, trueWindAngleStream) {
    var lastTrueWindTimestamp = trueWindAngleStream.map(function(){return Date.now();}).toProperty(0);
    ...
    return Bacon.mergeAll(calculated, trueWindAngleStream);
  }  
